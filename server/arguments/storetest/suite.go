...
func (suite *StoreTests) TestSaveIsLive() {
	original := acceptancetest.ParseSample(suite.T(), samplesPath+"save-request.json")
...
func (suite *StoreTests) TestUpdatedIsLive() {
	original := acceptancetest.ParseSample(suite.T(), samplesPath+"save-request.json")
	updated := acceptancetest.ParseSample(suite.T(), samplesPath+"update-request.json")
...
func (suite *StoreTests) TestFetchWithConclusionSearch() {
	store := suite.StoreFactory()
	sample := acceptancetest.ParseSample(suite.T(), samplesPath+"save-request.json")
...
func (suite *StoreTests) TestFetchWithOffset() {
	...
	first := acceptancetest.ParseSample(suite.T(), samplesPath+"save-request.json")
...
func (suite *StoreTests) TestFetchWithExclusions() {
	store := suite.StoreFactory()
	arg := acceptancetest.ParseSample(suite.T(), samplesPath+"save-request.json")
