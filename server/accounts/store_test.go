...
func (suite *StoreTests) TestNewUserFlow() {
	...
	account, err := store.NewResetToken(context.Background(), "email@soph.wiki")
...
func (suite *StoreTests) TestEmptyPasswordFailsBeforeInitialSet() {
	store := suite.StoreFactory()
	_, err := store.NewResetToken(context.Background(), "email@soph.wiki")
	require.NoError(suite.T(), err)
...
func (suite *StoreTests) TestSecondResetTokenInvalidatesFirst() {
	store := suite.StoreFactory()
	account1, err := store.NewResetToken(context.Background(), "email@soph.wiki")
	require.NoError(suite.T(), err)
	account2, err := store.NewResetToken(context.Background(), "email@soph.wiki")
